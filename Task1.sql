...
WITH 
  accessible_entities AS (
    ...
    FROM access
    WHERE hr_id = 1 AND right_code = 'Read'
)
...
FROM early_statuses ES
 ...
 WHERE 
   type_id = 1 AND 
   creation_date >= DATE_SUB(CURDATE(), INTERVAL 1 YEAR) AND 
   creation_date < CURDATE()   -- Є хоча б один статус із type_id = 1 (“Lead”) у вказаному (за останнй рік) діапазоні дат 
